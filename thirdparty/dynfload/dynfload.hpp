...
#else
        library = dlopen(path,((flags&LOCAL)==0?RTLD_GLOBAL:RTLD_LOCAL)|((flags&LAZY)==0?RTLD_NOW:RTLD_LAZY));
#endif
...
#if defined(_WIN32)
        FARPROC fprt = ::GetProcAddress(library,name);
        void* ptr = reinterpret_cast<void*>(fprt);
#else
